...
public final class MessagesModPlugin extends JavaPlugin {
  ...
  private final MessagesModTemplate service = new MessagesModTemplate();
  private final AtomicLong heartbeatTicks = new AtomicLong();
  private final ScheduledExecutorService scheduler =
      Executors.newSingleThreadScheduledExecutor(
          runnable -> {
            Thread thread = new Thread(runnable, "hd-messages-mod-worker");
  ...
  public MessagesModPlugin(JavaPluginInit init) {
    super(init);
  }
  ...
  protected void setup() {
    ...
    getCommandRegistry().registerCommand(new MessagesModStatusCommand());
    getCommandRegistry().registerCommand(new MessagesModDemoCommand());
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              try {
                ...
                if (tick % 60 == 0) {
                  getLogger().atInfo().log("[MessagesMod] heartbeat=%d", tick);
                }
              } catch (Exception exception) {
                lifecycle = Lifecycle.FAILED;
                service.incrementErrorCount();
                getLogger().atInfo().log("[MessagesMod] heartbeat failed: %s", exception.getMessage());
              }
            },
            1,
            1,
  ...
  private final class MessagesModStatusCommand extends CommandBase {
    private MessagesModStatusCommand() {
      super("hdmessagesmodstatus", "Shows runtime status for MessagesModPlugin.");
  ...
  private final class MessagesModDemoCommand extends CommandBase {
    private MessagesModDemoCommand() {
      super("hdmessagesmoddemo", "Runs a demo action for MessagesModPlugin.");
    setAllowsExtraArguments(true);
      this.setPermissionGroup(GameMode.Adventure);
    }
